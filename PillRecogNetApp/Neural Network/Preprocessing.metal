...
kernel void removeRGBMean(texture2d <half, access::read> inputTexture [[texture(0)]], texture2d <half, access::write> outputTexture [[texture(1)]], uint2 gid [[thread_position_in_grid]]) {
	...
	// Mean removal (multiplication by 255 because input image texture (RGBA8Unorm) scales values down between 0 and 1
	const auto inputColors = (half4(inputTexture.read(gid)) * 255.0h - means);
	const auto scaled = inputColors / 255.0h;	// Applied during training and fine tuning
